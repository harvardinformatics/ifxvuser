...
class RequestAPI {
  constructor(apiRoot, auth, defaultApprover) {
  ...
  setState(id, nextState, comment) {
    ...
    const data = { request_id: id, state: nextState, comment }
    return axios.post(
      url, data, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `${this.auth.authHeaderValue}`,
        },
      },
    )
  }
  ...
  updateAccountRequest(accountRequest) {
    ...
    const headers = {
      'Content-Type': 'application/json', Authorization: `${this.auth.authHeaderValue}`,
    }
  ...
  isUserApprover(request) {
    if (
      ...
      || !request.requestStates[0]
      || !request.requestStates[0].approvers
    ) return false
    return request.requestStates[0].approvers.includes(
